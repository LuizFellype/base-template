...
const useKeyboardListener = (handleCellClick, activeCell) => {
  useEffect(() => {
...
export default function App() {
  ...
  const hasClickedOnCellRef = useRef(false);
  ...
  // generate new active cell every INTERVAL timeframe.
  // handle Missed counting
  useEffect(() => {
    if (!isRunning) return;
    ...
    const intervalId = setInterval(() => {
      setActiveCell(() => {
        ...
        if (!hasClickedOnCellRef.current) {
          setScore((prevScore) => ({ ...prevScore, missed: prevScore.missed + 1 }));
        } 
        hasClickedOnCellRef.current = false
        return newCell;
      });
    }, INTERVAL);
  ...
  const handleCellClick = (cellIndex) => {
    hasClickedOnCellRef.current = true
  ...
  useKeyboardListener(handleCellClick, activeCell)
